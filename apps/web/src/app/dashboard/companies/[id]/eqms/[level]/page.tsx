...
export default function CompanyEqmsLevelPage() {
  ...
  async function uploadFile(file: File) {
    ...
    const ALLOWED = ['.pdf','.doc','.docx','.xls','.xlsx','.zip','.rar','.png','.jpg','.jpeg']
    const ext = '.' + file.name.split('.').pop()?.toLowerCase()
    if (file.size > MAX_SIZE) {
      alert(`File too large: ${(file.size/1024/1024).toFixed(1)}MB. Maximum allowed size is 10MB.`)
      return
    }
            ...
            {canEdit && selectedFolder && (
              isLevel5 ? (
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 10, color: '#9b9991', alignSelf: 'center' }}>PDF/DOCX/XLSX/ZIP · max 10MB</span>
                  <button onClick={() => {
