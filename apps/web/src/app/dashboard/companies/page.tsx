    ...
    <div>
      ...
      {loading ? (
        ...
        <div style={{ padding: 60, textAlign: 'center', color: '#9b9991', fontSize: 13 }}>
          No companies yet.{sessionRole === 'admin' ? ' Click "+ New company" to add one.' : ''}
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
          {companies.map(c => (
            <div key={c.id} onClick={() => router.push(`/dashboard/companies/${c.id}`)}
              style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,0.1)', borderRadius: 12, padding: '18px 20px', cursor: 'pointer', transition: 'box-shadow 0.15s' }}
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                ...
                <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(78,140,140,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, paddingTop: 12, borderTop: '0.5px solid rgba(0,0,0,0.06)' }}>
                <div>
                  <div style={{ fontSize: 10, color: '#9b9991', marginBottom: 2 }}>Members</div>
                  <div style={{ fontSize: 16, fontWeight: 500 }}>{c.member_count}</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: '#9b9991', marginBottom: 2 }}>TF Projects</div>
                  <div style={{ fontSize: 16, fontWeight: 500 }}>{c.project_count}</div>
                </div>
              </div>
            </div>
          ))}
