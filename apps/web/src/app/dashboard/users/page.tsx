...
export default function UsersPage() {
  ...
  async function deleteUser(user: User) {
    if (!confirm(`Permanently delete ${user.name}?\n\nThis cannot be undone. All their project and company memberships will be removed.`)) return
                  ...
                  <tr key={u.id} style={{ borderBottom: i < filtered.length - 1 ? '0.5px solid rgba(0,0,0,0.06)' : 'none', opacity: u.active ? 1 : 0.5 }}>
                    ...
                    <td style={{ padding: '11px 14px', textAlign: 'right' }}>
                      <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                        ...
                        <button onClick={() => deleteUser(u)} style={{ fontSize: 12, color: '#943030', background: 'transparent', border: '0.5px solid rgba(148,48,48,0.25)', borderRadius: 6, padding: '3px 10px', cursor: 'pointer' }}>Delete</button>
                      </div>
                    </td>
                  </tr>
                )
