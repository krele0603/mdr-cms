...
export default function ProjectDetailPage() {
  ...
  return (
    <div>
      ...
      {members.length > 0 && (
        <div style={{background:'#fff',border:'0.5px solid rgba(0,0,0,0.1)',borderRadius:12,padding:'12px 20px',marginBottom:14}}>
          <div style={{fontSize:12,fontWeight:500,color:'#6b6a64',marginBottom:8}}>Team</div>
          <div style={{display:'flex',flexWrap:'wrap' as const,gap:6}}>
            {members.map(m => {
              const rs = ROLE_STYLES[m.user_role] || ROLE_STYLES.client
              return (
                <div key={m.id} style={{display:'flex',alignItems:'center',gap:6,padding:'4px 10px',borderRadius:20,background:'#f8f7f4',border:'0.5px solid rgba(0,0,0,0.1)'}}>
                  <div style={{width:20,height:20,borderRadius:'50%',flexShrink:0,background:rs.bg,color:rs.color,border:`0.5px solid ${rs.border}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:8,fontWeight:600}}>
                    {m.name.split(' ').map((n:string) => n[0]).join('').toUpperCase().slice(0,2)}
                  </div>
                  <span style={{fontSize:11,fontWeight:500}}>{m.name}</span>
                  <span style={{fontSize:10,color:'#9b9991'}}>{m.user_role}</span>
                </div>
              )
            })}
          </div>
