#!/usr/bin/env python3
"""
Adds template version upgrade dropdown to document editor meta strip.
Consultants/admins can switch template version per document.
"""
...
# 1. Add state for template versions
if 'templateVersions' not in content:
  ...
  const [showRequestChanges, setShowRequestChanges] = useState(false)"""
    ...
    fixes += 1
    print('Added templateVersions state')

# 2. Load template versions when doc loads
if 'upgrade-template' not in content:
    content = content.replace(
        "        setLoading(false)\n        loadExamples()",
        """        setLoading(false)
        loadExamples()
        // Load available template versions for upgrade
        if (['admin', 'consultant'].includes(data.created_by_role || '') || true) {
    ...
    fixes += 1
    print('Added template versions load on doc load')

# 3. Add upgrade template function
if 'upgradeTemplate' not in content:
    content = content.replace(
  ...
  async function reviseDoc() {"""
    ...
    fixes += 1
    print('Added upgradeTemplate function')

# 4. Add template version UI to meta strip (after template_name display)
if 'showVersionUpgrade' not in content:
    content = content.replace(
    ...
    fixes += 1
    print('Added template version upgrade UI to meta strip')
